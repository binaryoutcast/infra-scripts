...
foreach ($glob as $_value) {
  ...
  $_addon['type'] = 8;
  $_addon['slug'] = strtolower($_manifest['slug']);
  $_addon['owner'] = 2;
  $_addon['targetClient'] = $_targetApplication;
  $_addon['name'] = $_manifest['name']['en-US'];
  $_addon['description'] = $_manifest['description']['en-US'];
  $_addon['category'] = 0;
  $_addon['tags'] = 'strings locale langpack language pack palemoon pale moon firefox';
  $_addon['addonURL'] = gfBuildPath('addon', strtolower($_manifest['slug']));

  $_xpi['addonID'] = $_manifest['id'];
  $_xpi['targetClient'] = $_targetApplication;
